...
if __name__ == "__main__":
    ...
    if "4" in argv:
        # train vae
        input_crop_paths = sorted(list(glob.glob(os.path.join(paths["output_dir"], "corner_crops", "*_binary.npy"))))
        ...
        train_vae_outlier_detector(input_crop_paths, paths["output_dir"], vae_config)

    if "5" in argv:
        # forward vae
        model_path = os.path.join(paths["output_dir"], "vae_outlier_detector", "vae_model.pt")
        run_vae_outlier_detector(input_crop_paths, paths["output_dir"], model_path, vae_config)

    if "6" in argv:
        # determine outliers
        outlier_path = os.path.join(paths["output_dir"], "vae_outlier_detector", "outliers.json")
        determine_outliers(paths["output_dir"], save_path=outlier_path, thres_loss_percent=vae["outlier_thres"], save_imgs=True)

    if "7" in argv:
        # initial camera calibration (PnP)
        calib_initial_params(paths["output_dir"], config["calib_initial"], chb, outlier_path=outlier_path)

    if "8" in argv:
        # initial world points
        estimate_initial_world_points(paths["output_dir"], config["calib_initial"], chb)
